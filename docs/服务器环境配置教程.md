...
回到数据盘文件夹（us_loan),
``` 
git clone https://github.com/quanttide/us-loan-scraper.git
```
...
```
# 使用 pip 安装（通常兼容性更好，且包含 lxml）
pip install pandas tqdm nltk beautifulsoup4 lxml
```

运行代码：

```
nohup python us-loan-scraper/src/main.py > output.log 2>&1 &
```
